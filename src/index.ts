...
class AutoCode {
  ...
  private async processRequest(request: CodeRequest): Promise<void> {
    ...
    try {
      ...
      // Step 4: Execute Claude CLI (two-phase: analysis + implementation)
      console.log('\n[Step 4] Executing Claude CLI (two-phase process)...');
...
*This merge request was automatically generated by AutoCode.*`,
      ...
      // Step 8: Mark as processed in persistent storage
      await this.storage.markProcessed(request.id);

      // Step 9: Notify Discord
      console.log('\n[Step 9] Notifying Discord...');
      await this.discord.replyToMessage(
        request.channelId,
        request.messageId,
